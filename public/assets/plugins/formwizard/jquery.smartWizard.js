...
;(function ($, window, document, undefined) {
    ...
    var defaults = {
        ...
        contentURL: null, // content url, Enables Ajax content loading. Can also set as data data-bs-content-url on anchor
    ...
    $.extend(SmartWizard.prototype, {
        ...
        _setToolbar: function () {
            ...
            var btnPrevious = this.options.toolbarSettings.showPreviousButton !== false ? $('<button></button>').text(this.options.lang.previous).addClass('btn btn-light sw-btn-prev').attr('type', 'button') : null;
            var btnGroup = $('<div></div>').addClass('btn-group m-1 sw-btn-group').attr('role', 'group').append(btnPrevious, btnNext);

            // Add extra toolbar buttons
            var btnGroupExtra = null;

            if (this.options.toolbarSettings.toolbarExtraButtons && this.options.toolbarSettings.toolbarExtraButtons.length > 0) {
                btnGroupExtra = $('<div></div>').addClass('btn-group m-1 sw-btn-group-extra').attr('role', 'group');
